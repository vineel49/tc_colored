% log-BCJR algorithm (for the final iteration)
% outputs aposteriori information
% BCJR algorithm in the probability-domain is given in
% http://home.iitk.ac.in/~vasu
function [LLR]= log_BCJR_END(LLR,log_gamma,num_bit)
[P_State,P_Ip,Ga_Inx,N_State,Gb_Inx,N_Ip]= Get_Trellis_Pred();
...
%**************************************************************************
% Compute aposteriori LLR
%**************************************************************************
 temp5 = log_alpha + log_gamma(Gb_Inx(:,1),:)+repmat(LLR/2,num_states,1) + log_beta(N_State(:,1),2:num_bit+1) ;
 ...
 temp6 = log_alpha + log_gamma(Gb_Inx(:,2),:)-repmat(LLR/2,num_states,1)+ log_beta(N_State(:,2),2:num_bit+1) ;
